...
public class AzureStorageSecureCredentialsControllerService
    ...
    public AzureStorageCredentialsDetails getStorageCredentialsDetails(Map<String, String> attributes) {
        ...
        if (StringUtils.isBlank(accountNameValue)) {
            throw new IllegalArgumentException(String.format(
                    "Storage Account Name is not set in keyvault secret '%s'.", accountNameSecret));
        }
        ...
        if (StringUtils.isNotBlank(accountKeySecret)) {
            ...
            if (StringUtils.isBlank(accountKeyValue)) {
                throw new IllegalArgumentException(String.format(
                        "Storage Account Key is not set in keyvault secret '%s'.", accountKeySecret));
            }
            ...
            if (StringUtils.isBlank(sasTokenValue)) {
                throw new IllegalArgumentException(String.format(
                        "Storage Account SAS Token is not set in keyvault secret '%s'.", sasTokenSecret));
            }
